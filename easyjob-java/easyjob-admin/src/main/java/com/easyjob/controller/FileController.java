...
public class FileController extends ABaseController {
    ...
    /**
     * 该接口/getImage/{imageFolder}/{imageName}的主要作用是根据指定的文件夹路径和文件名，读取并返回图片文件内容。具体功能如下：
     * 调用readImage方法，验证传入的imageFolder和imageName是否为空。
     * 拼接图片的完整路径，并设置响应的内容类型为图片类型（如image/jpeg）。
     * 调用readFile方法读取图片文件内容，并将其写入到HTTP响应流中。
     * 设置缓存控制头，允许浏览器缓存图片2592000秒（30天）。
     */
