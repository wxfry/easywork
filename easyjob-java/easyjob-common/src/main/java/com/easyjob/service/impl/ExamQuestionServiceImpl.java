...
public class ExamQuestionServiceImpl implements ExamQuestionService {
    ...
    public List<ImportErrorItem> importExamQuestion(MultipartFile file, SessionUserAdminDto userAdminDto) {
        List<Category> categoryList = categoryService.loadAllCategoryByType(CategoryTypeEnum.QUESTION_EXAM.getType());
        ...
        for (List<String> row : dataList) {
            ...
            if (StringTools.isEmpty(title) || title.length() > Constants.LENGTH_300) {
                errorItemList.add("标题不能为空，且长度不能超过" + Constants.LENGTH_300);
            }
            ...
            String teacher = row.get(index++);
            String knowledgePoints = row.get(index++);
            String origin = row.get(index++);
            String chapter = row.get(index++);
            ...
            examQuestion.setCategoryName(category.getCategoryName());
            examQuestion.setTeacher(teacher);
            examQuestion.setKnowledgePoints(knowledgePoints);
            examQuestion.setorigin(origin);
            examQuestion.setchapter(chapter);
...
//        return this.appExamMapper.selectByChapterAndCategoryName(chapter, categoryName);
        return this.examQuestionMapper.selectByChapterAndCategoryName(chapter, categoryName);
    }
