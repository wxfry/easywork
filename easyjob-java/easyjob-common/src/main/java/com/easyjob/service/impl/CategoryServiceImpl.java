...
public class CategoryServiceImpl implements CategoryService {
    ...
    public List<Category> findClassifyByParam(CategoryQuery param) {
        return this.categoryMapper.selectClassify(param);
    }
    ...
    public List<Category> loadAllCategoryByType(Integer type) {
        ...
        // 这句注释是设置查询类型：
        // 将 typeEnum.getType() 和 CategoryTypeEnum.QUESTION_EXAM.getType() 组成的数组作为查询条件，确保查询结果包含这两种类型的分类。
//        categoryQuery.setTypes(new Integer[]{typeEnum.getType(), CategoryTypeEnum.QUESTION_EXAM.getType()});
        categoryQuery.setTypes(new Integer[]{typeEnum.getType()});
        return this.categoryMapper.selectList(categoryQuery);
    }

    @Override
    public PaginationResultVO<Category> findClassify(CategoryQuery param) {
        int count = this.findCountByParam(param);
        int pageSize = param.getPageSize() == null ? PageSize.SIZE15.getSize() : param.getPageSize();

        SimplePage page = new SimplePage(param.getPageNo(), count, pageSize);
        param.setSimplePage(page);
        List<Category> list = this.findClassifyByParam(param);
        PaginationResultVO<Category> result = new PaginationResultVO(count, page.getPageSize(), page.getPageNo(), page.getPageTotal(), list);
        return result;
    }
}