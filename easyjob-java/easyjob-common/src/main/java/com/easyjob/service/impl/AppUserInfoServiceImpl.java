...
public class AppUserInfoServiceImpl implements AppUserInfoService {
    ...
    public String login(String email, String password, String ip, String deviceId, String deviceBrand) {
        ...
        Date lastLoginTime = appUserInfo.getLastLoginTime();
        if (lastLoginTime != null) {
            // 检查最后登录日期是否是当前日期的前一天
            Calendar lastLoginCalendar = Calendar.getInstance();
            lastLoginCalendar.setTime(lastLoginTime);
            lastLoginCalendar.add(Calendar.DAY_OF_YEAR, 1);
            Date nextDayOfLastLogin = lastLoginCalendar.getTime();
            ...
            Date startOfDayCurrentDate = currentCalendar.getTime();

//            if (nextDayOfLastLogin.equals(startOfDayCurrentDate)) {
//                // 如果是前一天，则signIn加1
//                appUserInfo.setSignIn(appUserInfo.getSignIn() != null ? appUserInfo.getSignIn() + 1 : "1");
//            } else {
//                // 否则signIn设为0
//                appUserInfo.setSignIn("0");
//            }
//        } else {
//            // 如果没有最后登录日期，则signIn设为0
//            appUserInfo.setSignIn("0");
        }
...
//        updateInfo.setSignIn(appUserInfo.getSignIn());
