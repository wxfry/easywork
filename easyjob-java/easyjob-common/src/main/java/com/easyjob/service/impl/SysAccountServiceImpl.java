...
import com.easyjob.entity.enums.MenuTypeEnum;
...
public class SysAccountServiceImpl implements SysAccountService {
    ...
    /**
     * 根据UserId获取对象
     * mapper层使用select, service层使用get, 实际调用的东西是一样的,只是同样的方法在不同的层名字不一样
     * 虽然不会报错, 但是不规范
     * mapper操作dv的就用select, service层使用get,find,query
     */
    @Override
    ...
    public SessionUserAdminDto login(String phone, String password) {
        ...
        if (!StringTools.isEmpty(appConfig.getSuperAdminPhones()) &&
            ...
            query.setFormate2Tree(false); // 不需要格式化成树形结构
