...
public class SearchController extends ABaseController {
    ...
    @GlobalInterceptor(frequencyType = RequestFrequencyTypeEnum.MINUTE, requestFrequencyThreshold = 20)
    public ResponseVO searchQuestion(@VerifyParam(required = true) Integer questionId) {
        ExamQuestionQuery query = new ExamQuestionQuery();
        query.setQuestionId(questionId);
        PaginationResultVO resultVO = examQuestionService.findListByPage(query);
//        List<ExamQuestion> examQuestionList = resultVO.getList();
//        List<String> objectIdList = examQuestionList.stream().map(item -> item.getQuestionId().toString()).collect(Collectors.toList());
//        if (objectIdList.isEmpty()) {
//            return getSuccessResponseVO(resultVO);
//        }
//        Map<Integer, ExamQuestion> objectIdMap = examQuestionList.stream()
