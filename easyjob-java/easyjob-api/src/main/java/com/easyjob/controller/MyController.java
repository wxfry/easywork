...
//    }
    ...
    public ResponseVO loadWrongQuestion(@RequestHeader(value = "token", required = false) String token,
        ...
        // 调用 findListByPage 方法，该方法已经实现了去重逻辑
        PaginationResultVO resultVO = appExamQuestionService.findListByPage(appExamQuestionQuery);

        List<AppExamQuestion> appExamQuestionList = resultVO.getList();

        List<String> questionIds = appExamQuestionList.stream()
                .map(item -> item.getQuestionId().toString())
    ...
    @RequestMapping("/createWrongText")
    @GlobalInterceptor(checkLogin = true)
    public ResponseVO createWrongText(@RequestHeader(value = "token", required = false) String token,
                                      @VerifyParam(required = true) Integer questionCount,
                                      @VerifyParam(required = true) String practiceMode) {
        AppUserLoginDto appDto = getAppUserLoginfoFromToken(token);
        return getSuccessResponseVO(appExamService.createWrongText(practiceMode, questionCount, appDto));
    }
}