<template>
  ...
  >
    ...
    >
      <el-row :gutter="10">
        <el-col :span="12">
          <el-form-item label="标题" prop="title" style="display: flex; align-items: center;">
            <el-input :maxLength="10" v-model="formData.title" style="flex: 1;" />
            <span class="iconfont icon-upload" @click="handleUpload('title')" style="margin-left: 10px;"></span>
          </el-form-item>
          ...
          <el-form-item label="老师" prop="teacher">
            <el-input :maxLength="10" v-model="formData.teacher" />
          </el-form-item>
          <el-form-item label="知识点" prop="knowledgePoints">
            <el-input :maxLength="10" v-model="formData.knowledgePoints" />
          </el-form-item>
          <el-form-item label="来源" prop="origin">
            <el-input :maxLength="10" v-model="formData.origin" />
          </el-form-item>
          <el-form-item label="章节" prop="chapter">
            <el-input :maxLength="10" v-model="formData.chapter" />
          </el-form-item>
        ...
        <el-col :span="12">
          <el-form-item label="问题描述" prop="question">
            ...
            <span class="iconfont icon-upload" @click="handleUpload('question')" style="margin-left: 10px;"></span>
          </el-form-item>

          <el-form-item label="答案分析" prop="answerAnalysis">
            <SunEditor v-model="formData.answerAnalysis" :height="200"></SunEditor>
            <span class="iconfont icon-upload" @click="handleUpload('answerAnalysis')" style="margin-left: 10px;"></span>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
...
<script setup>
import { LETTER, QUESTION_TYPE, KNOWLEDGE_POINT } from "@/utils/Constants.js";
...
const handleUpload = (field) => {
  ...
  input.type = 'file';
  input.accept = 'image/*';
  input.onchange = (event) => handleFileChange(event, field);
  input.click();
};
...
const handleFileChange = async (event, field) => {
  ...
  if (field === 'question') {
    formData.value.question = ocrResult;
  } else if (field === 'answerAnalysis') {
    formData.value.answerAnalysis = ocrResult;
  } else if (field === 'title') {
    formData.value.title = ocrResult;
  }
};
...
const rules = {
  ...
  teacher: [{ required: true, message: "请输入老师"}],
  knowledgePoints: [{ required: true, message: "请输入知识点"}],
  origin: [{ required: true, message: "请输入来源"}],
